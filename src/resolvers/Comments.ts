...
export class CommentsResolver {
  ...
  async createComment(
    ...
    const post: Post = await datasource
      .getRepository(Post)
      .findOne({ where: { id }, relations: { comments: true } });

    if (user) {
      const comment = { ...data, user, post, created_at: new Date() };
  ...
  async deleteComment(
    ...
    const comment = await datasource
      ...
      .findOne({
        where: { id },
        relations: { user: true, post: { blog: { user: true } } },
      });
    if (comment === null) {
      throw new Error("Ce commentaire n'existe pas");
    }
